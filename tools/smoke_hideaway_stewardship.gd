...
func _init() -> void:
	...
	print("Archive Hideaway stewardship smoke test passed: short trips cannot farm rewards; qualifying active-play expeditions bank bounded opportunities; salvage restores facilities; preparations consume exactly once; durable state round trips; and elapsed time alone grants nothing.")
	quit(0)
