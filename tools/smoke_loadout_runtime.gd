...
func run_smoke_test() -> void:
	...
	check(int(validation.get("equipment_item_count", 0)) == 4, "Reference campaign must expose four equipment items.")
	...
	if script_value is GDScript:
		check(str((script_value as GDScript).resource_path) == "res://src/equipment_runtime.gd", "Runtime scene must bind the equipment-aware runtime.")
