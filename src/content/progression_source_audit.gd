...
static func audit(
	...
	var non_recipe_supply_value: Variant = supply_profile.get("non_recipe_supply", {})
	var available_recipe_items_value: Variant = supply_profile.get("available_recipe_items", {})
	var non_recipe_supply: Dictionary = (
		non_recipe_supply_value as Dictionary
		if typeof(non_recipe_supply_value) == TYPE_DICTIONARY
		else {}
	)
	var available_recipe_items: Dictionary = (
		available_recipe_items_value as Dictionary
		if typeof(available_recipe_items_value) == TYPE_DICTIONARY
		else {}
	)
