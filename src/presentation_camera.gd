...
func _process(delta: float) -> void:
	...
	var deadzone := PresentationCatalog.number(profile, "camera", "deadzone", 20.0)
	var follow_strength := PresentationCatalog.number(profile, "camera", "follow_strength", 8.0)
...
func desired_camera_offset(runtime: Node) -> Vector2:
	...
	var look_ahead := PresentationCatalog.number(active_profile(), "camera", "look_ahead", 18.0)
