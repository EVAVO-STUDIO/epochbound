...
static func expand_recipe_requirements(
	...
	# Resolve each original demand against a shared finite-supply pool. Ingredients
	# are expanded only for the residual quantity that genuinely must be crafted.
...
static func expand_recipe_demand(
	...
	if quantity <= 0:
		return
	var residual_quantity := consume_non_recipe_supply(item_id, quantity, remaining_supply)
...
static func consume_non_recipe_supply(
	item_id: String,
	quantity: int,
	remaining_supply: Dictionary
) -> int:
