...
func run_test() -> void:
	...
	check(int(validation.get("presentation_profile_count", 0)) == 8, "Reference campaign must expose eight presentation profiles.")
	check(int(validation.get("presentation_binding_count", 0)) == 8, "Reference campaign must bind every map and era.")
	...
	var hideaway_verdant: Dictionary = PresentationCatalog.resolved_profile(definitions, bindings, "archive_hideaway", "verdant")
	var hideaway_ashen: Dictionary = PresentationCatalog.resolved_profile(definitions, bindings, "archive_hideaway", "ashen")
	check(str(hideaway_verdant.get("id", "")) == "hideaway_verdant", "Verdant Hideaway must resolve its quiet authored profile.")
	check(str(hideaway_ashen.get("id", "")) == "hideaway_ashen", "Ashen Hideaway must resolve its quiet authored profile.")
