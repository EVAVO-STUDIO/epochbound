...
static func collect_story_required_item_evidence(
	...
	# execute sequentially. Traverse the graph so two mutually exclusive choices
	# do not look like two mandatory consumptions.
...
static func conversation_node_evidence(
	...
	if visiting.has(node_id):
		# Cycles are bounded here. Content validation owns graph legality; the
		# quantity probe must never inflate a loop into infinite required supply.
		return empty_evidence()
...
static func required_items_from_evidence(held: Dictionary, consumed: Dictionary) -> Dictionary:
	...
	for item_id in sorted_dictionary_keys(all_ids):
		...
		# physical item, so do not add those quantities together. Repeated
		# sequential removals are cumulative because each consumes real supply.
		output[item_id] = maxi(
...
static func expand_recipe_requirements(
	...
	# and handles an item that is both directly required and used by a recipe.
