...
func _init() -> void:
	for path in PATHS:
		...
		if resource == null:
			...
			quit(1)
			return
	print("Archive Hideaway runtime compile probe passed: stewardship, derived mementos, optional quiet moments, live root bridge, save semantics, map and dedicated regressions load under Godot 4.6.2.")
	quit(0)
