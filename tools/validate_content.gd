...
func _initialize() -> void:
	var report := CombatDirectorValidator.validate_all()
	for warning in report.get("warnings", []):
		print_rich("[color=yellow]WARNING[/color] %s" % warning)
	for error in report.get("errors", []):
		push_error(str(error))
	print(
		"Content validation: %d campaign(s), %d map(s), %d definition(s), %d placement(s), %d encounter zone(s), %d warning(s), %d error(s)." % [
