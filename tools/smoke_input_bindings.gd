...
func test_atomic_binding_persistence() -> void:
	...
	check(FileAccess.file_exists(PlayerSettingsStore.settings_path(TEST_ROOT)), "Rejected control writes must leave the valid primary settings file in place.")
	check(not FileAccess.file_exists(PlayerSettingsStore.backup_path(TEST_ROOT)), "Rejected control writes must not rotate the valid primary settings file into a backup.")
	...
	check(not bool(after_rejection.get("recovered_from_backup", false)), "A rejected control write must continue loading directly from the unchanged primary file.")
