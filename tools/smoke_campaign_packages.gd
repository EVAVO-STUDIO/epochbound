...
func run_test() -> void:
	...
	check(bool(report.get("ok", false)), "Reference campaign must pass the strict Sprite Animation release validation chain.")
	...
	if bool(install.get("ok", false)):
		...
		check(bool(installed_validation.get("ok", false)), "Installed campaign must pass the strict Sprite Animation validator.")
...
func finish() -> void:
	if failures.is_empty():
		print("Campaign package smoke test passed: deterministic export, manifest inspection, strict Sprite Animation validation, atomic install and replacement are coherent.")
		quit(0)
		return
